...
// TagFilter represents a technology-agnostic tag filter expression.
// Core constructs this from suite selectors; adapters translate it
// to their technology-specific syntax (godog, cucumber-js, etc.).
type TagFilter struct {
	// Selectors are OR'd together. Each selector is an AND/OR/NOT expression.
	Selectors []TagFilterSelector
}

// TagFilterSelector represents one selector within a tag filter.
type TagFilterSelector struct {
	RequireTags []string // AND: must have all
	AnyOfTags   []string // OR: must have at least one
	ExcludeTags []string // NOT: must not have any
}

// TagFilterTranslator converts a generic TagFilter to a technology-specific
// tag expression string.
type TagFilterTranslator interface {
	TranslateTagFilter(filter TagFilter) string
}
