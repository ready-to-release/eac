# CLIE CLI Configuration Reference

This document provides comprehensive documentation for the CLIE CLI configuration file (`clie-cli.yml`).
...
The CLIE CLI uses a YAML configuration file to define containerized extensions that extend the CLI's capabilities.
...
| 2        | User-Specific Config  | `.clie/clie-cli.local.yml`, `.clie/clie-cli.personal.yml`, etc. |
| 3        | Repository Config     | `.clie/clie-cli.yml`                                          |
...
```yaml
defaults:
  registry: "ghcr.io/ready-to-release/clie"
...
extensions:
  - name: "eac"
    image: "ghcr.io/ready-to-release/eac-ext:sha-abc123"
...
| ------------------------ | --------------- | ----------------------------------------- |
| `CLIE_CONTAINER_REPOROOT` | `/var/task`     | Repository root path inside the container |
| `CLIE_HOST_REPOROOT`      | `<actual path>` | Repository root path on the host          |
...
| File                          | Purpose                                        |
| ----------------------------- | ---------------------------------------------- |
| `.clie/clie-cli.local.yml`      | Local development overrides (highest priority) |
| `.clie/clie-cli.personal.yml`   | Personal customizations                        |
| `.clie/clie-cli.dev.yml`        | Development environment settings               |
| `.clie/clie-cli.<username>.yml` | User-specific settings                         |

### Override Example

Base config (`.clie/clie-cli.yml`):
...
Local override (`.clie/clie-cli.local.yml`):
...
.clie/clie-cli.personal.yml
.clie/clie-cli.dev.yml
.clie/clie-cli.*.yml
!.clie/clie-cli.yml
```
...
```text
contracts/clie-cli/0.1.0/clie-cli.schema.json
```
...
- [JSON Schema](./clie-cli.schema.json) - Formal schema definition
