...
| schemas/clie-cli.schema.json | JSON Schema for clie-cli.yml validation |
| schemas/command.ebnf | EBNF grammar for CLI command parsing |
| schemas/clie-cli.yml.md | Reference documentation for CLI configuration |
...
The `cliecli` package (moniker: contracts-clie-cli) provides the embedded
