# Changelog

Changelog format and versioning conventions for repository-level and module-level change documentation.

## Overview

The repository uses [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) format with [Semantic Versioning](https://semver.org/spec/v2.0.0.html) for both repository-level and module-level changelogs.

## In This Section

| Reference                                         | Description                               |
| ------------------------------------------------- | ----------------------------------------- |
| [Release Commands](./release-commands.md)         | CLI commands for the release workflow     |
| [Format Specification](./format-specification.md) | Keep a Changelog format details           |
| [Repository Changelog](./repository-changelog.md) | Repository-level changelog conventions    |
| [Module Changelogs](./module-changelogs.md)       | Module-level changelog conventions        |
| [Versioning](./versioning.md)                     | Semantic versioning rules and conventions |
