...
**Solution**: Use `ci summary-link` to generate markdown diagnostics optimized for GitHub's `$GITHUB_STEP_SUMMARY` environment.
...
r2r eac ci summary-link 12345678 --type build --artifact build-logs

# Test failure diagnostic
r2r eac ci summary-link 12345678 --type test --artifact test-results

# Container build failure
r2r eac ci summary-link 12345678 --type container --image myapp:latest

# Release validation failure
r2r eac ci summary-link 12345678 --type release --workflow release --commit abc123

# Documentation build failure
r2r eac ci summary-link 12345678 --type docs --artifact docs-site

# Contract deviation
r2r eac ci summary-link 12345678 --type deviation
```
...
```bash
r2r eac ci summary-link <run-id> [options]
...
r2r eac ci summary-link 12345678 --type build --artifact build-logs
r2r eac ci summary-link 12345678 --type test --artifact test-results
r2r eac ci summary-link 12345678 --type container --image api:v1.2.3
r2r eac ci summary-link 12345678 --type release --workflow release --commit abc123
r2r eac ci summary-link 12345678 --type docs --artifact mkdocs-site
r2r eac ci summary-link 12345678 --type deviation
```
  ...
  build:
    ...
    steps:
      ...
      - name: Generate build summary
        ...
        run: |
          r2r eac ci summary-link ${{ github.run_id }} \
            --type build \
            --artifact build-logs \
            >> $GITHUB_STEP_SUMMARY
    ...
    steps:
      ...
      - name: Generate diagnostic summary
        run: |
          ...
          r2r eac ci summary-link ${{ github.run_id }} $ARGS \
            >> $GITHUB_STEP_SUMMARY
...
r2r eac ci summary-link 12345678 --type build --artifact custom-logs
...
if [ $? -ne 0 ]; then
  ...
  r2r eac ci summary-link 0 --type deviation
  exit 1
fi
```
...
ci-test:
  @r2r eac test || \
    (r2r eac ci summary-link $(RUN_ID) --type test --artifact test-results && exit 1)
```
...
RUN r2r eac build || \
    r2r eac ci summary-link ${RUN_ID} --type build --artifact build-logs
```
...
r2r eac ci summary-link 12345678 --type build --artifact logs > summary.md
...
r2r eac ci summary-link $RUN_ID --type build --artifact build-logs >> $GITHUB_STEP_SUMMARY
echo "" >> $GITHUB_STEP_SUMMARY

r2r eac ci summary-link $RUN_ID --type test --artifact test-results >> $GITHUB_STEP_SUMMARY
```
...
3. Run: `r2r eac ci summary-link <run-id> --type <type> [options]`
...
The CI command enhances GitHub Actions workflows with rich, context-aware diagnostic summaries that accelerate debugging and resolution.
