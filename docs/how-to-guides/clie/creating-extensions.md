...
You want to add custom automation platforms to clie that can be shared across teams and run consistently in any environment.

**Solution**:

Create a Docker-based clie extension that clie invokes as a container.

## How Extensions Work

clie extensions are Docker containers that:
...
This provides clie CLI with extension information for discovery, validation, and management.

### Why It Matters

The extension metadata command allows:

- clie CLI to discover extension capabilities
...
```bash
clie my-ext do-something --flag value
...
```bash
# Via run command
clie run my-ext do-something

# Via alias (created automatically)
clie my-ext do-something
```
...
```yaml
extensions:
  - name: string              # Required: Command name (clie <name>)
...
### Integration Testing via clie CLI
...
repoRoot := os.Getenv("CLIE_CONTAINER_REPOROOT") // "/var/task"
configPath := filepath.Join(repoRoot, ".clie", "eac", "repository.yml")
```
...
| ------------------------- | ----------------------------------- |
| `CLIE_CONTAINER_REPOROOT` | Repository mount path (`/var/task`) |
| `CLIE_HOST_REPOROOT`      | Original host path                  |
| `CLIE_ENV`                | Environment identifier              |
| `CLIE_OPERATION_ID`       | Unique operation ID for tracing     |
...
```bash
# Install clie CLI with eac-ext
clie install eac

# Build
clie eac build my-extension

# Test
clie eac test my-extension

# Validate
clie eac validate dependencies
clie eac validate specs

# Release management
clie eac release pending my-extension
clie eac release this my-extension
```
