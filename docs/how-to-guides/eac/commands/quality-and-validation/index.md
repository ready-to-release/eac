# Quality and Validation

{{ page_breadcrumb() }}

Learn how to validate code quality and scan for issues before committing.

## In This Section

| Guide | What You'll Accomplish |
|-------|------------------------|
| [Validate Before Commit](./validate-before-commit.md) | Run comprehensive quality checks |
| [Scan for Security Issues](./scan-for-security-issues.md) | Detect vulnerabilities and secrets |
| [Check Dependencies](./check-dependencies.md) | Verify dependency contracts |
| [Validate Specifications](./validate-specifications.md) | Check Gherkin specs for quality |

## Quality Gates

### Pre-Commit Validation

- [Contract validation](./validate-before-commit.md)
- [Dependency checking](./check-dependencies.md)
- [Specification validation](./validate-specifications.md)
- [Security scanning](./scan-for-security-issues.md)

### Integration Points

- Pre-commit hooks
- CI/CD pipelines
- Release gates

{{ diataxis_footer() }}
