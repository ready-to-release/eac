...
**Generated by**: `eac release this <module>` from conventional commits
...
modules:
  - moniker: eac-ext
    ...
    versioning:
      scheme: SemVer
    changelog: ../../release/eac-ext/CHANGELOG.md # Relative from containers/eac-ext
    components:
      dockerfile:
        root: containers/eac-ext
```

**Why relative?** When component root is deep in the repository structure (`containers/eac-ext`), a relative path maintains the connection to the centralized `release/` folder.
...
- `eac release this <module>` - Updates changelog
- `eac show changelog <module>` - Displays changelog
- `eac get changelog <module>` - Exports changelog data
- `eac validate release <module>` - Validates changelog format
...
### Container Module with Relative Path (eac-ext)
...
**Changelog location**: `release/eac-ext/CHANGELOG.md` (resolved from `containers/eac-ext`)
...
When you run `eac release this <module>`, manual entries are preserved and merged with auto-generated entries.
