        ...
        try {
            ...
            try {
                ...
                commitMessage = await vscode.window.withProgress({
                    ...
                    // Start the actual commit execution with real progress callback
                    const agentPromise = executeAgent(workspacePath, (realProgress) => {
                        log('[commit Progress] ' + realProgress);
...
async function executeAgent(workspacePath: string, onProgress?: (message: string) => void): Promise<string> {
    return new Promise((resolve, reject) => {
        // Execute commit command directly (replaces old MCP server approach)
        ...
        // Call commit which handles everything: generation, cleanup, validation, auto-fix
        const childProcess = child_process.spawn('go', ['run', '.', 'commit'], {
            cwd: commandsPath,
        ...
        childProcess.on('close', (code) => {
            ...
            if (!commitMessage) {
                ...
                reject(new Error(`commit failed: ${errorMsg}`));
                return;
            }
        ...
        // No input needed for commit (it reads git directly)
        childProcess.stdin.end();
    });
}

/**
 * Extracts the commit message from commit output
 * Format:
