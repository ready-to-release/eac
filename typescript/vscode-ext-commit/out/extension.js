...
const stable_status_bar_1 = require("./stable-status-bar");
...
function activate(context) {
    ...
    stableStatusBar = new stable_status_bar_1.StableStatusBar(context);
        ...
        const updateButtonState = () => {
            ...
            if (git.repositories.length > 0) {
                ...
                if (indexChanges.length > 0) {
                    debugInfo.push(`  Staged files: ${indexChanges.map((c) => c.uri.fsPath).join(', ')}`);
                }
    ...
    let disposable = vscode.commands.registerCommand('vscode-ext-commit.callMCP', async () => {
        ...
        try {
            ...
            if (existingMessage && existingMessage.length > 0) {
                ...
                stableStatusBar.stopGeneration();
                vscode.commands.executeCommand('setContext', 'vscode-ext-commit.isGenerating', false);
                const overwrite = await vscode.window.showWarningMessage('Generating a new message will overwrite the existing', { modal: false }, 'OK');
                if (overwrite !== 'OK') {
 ...
 */
async function validateGitState(repo) {
...
async function executeAgent(workspacePath, onProgress) {
 ...
 */
function extractCommitMessageFromOutput(output) {
