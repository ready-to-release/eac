...
func TestConcurrentPortReservation(t *testing.T) {
	...
	reservedPorts := make(map[int]int) // port -> count
	for port := range portsChan {
		reservedPorts[port]++
	}

	// Assert no port was reserved more than once
	for port, count := range reservedPorts {
	...
	assert.Equal(t, numGoroutines*portsPerGoroutine, len(reservedPorts),
		"Expected %d unique ports, got %d", numGoroutines*portsPerGoroutine, len(reservedPorts))
