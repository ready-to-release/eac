...
// WorkflowRunInfo represents workflow run information from GitHub
...
func PipelineStatus() int {
	...
	// Get commit info
	commitInfo, err := getCommitInfo(commitSHA)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Warning: failed to get commit info: %v\n", err)
		commitInfo = &CommitInfo{SHA: commitSHA, Message: "(unknown)", Author: "(unknown)"}
	}
	...
	latestRuns := make(map[string]*WorkflowRunInfo)
	for i := range runs {
		run := &runs[i]
...
// getCommitInfo gets commit information
...
func getWorkflowRunsForCommit(sha string) ([]WorkflowRunInfo, error) {
