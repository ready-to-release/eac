...
//
// It checks in order:
// 1. R2R_MOCK_AI_<COMMAND> env var for a specific mock file name
// 2. R2R_MOCK_AI_DIR/<command>/mock-response.{txt,md,json} as defaults
//
...
func GetMockResponse(command string) (string, bool) {
	...
	// e.g., R2R_MOCK_AI_SPECS=mock-response-conflict.txt
...
//
// It checks in order:
// 1. R2R_MOCK_AI_<COMMAND>_<SUBCOMMAND> env var
// 2. R2R_MOCK_AI_DIR/<command>/mock-<subcommand>-response.{txt,md,json}
...
func GetMockResponseWithSubcommand(command, subcommand string) (string, bool) {
	...
	// e.g., R2R_MOCK_AI_RISKS_ASSESSMENT=custom-assessment.md
	envKey := EnvMockAIPrefix + strings.ToUpper(strings.ReplaceAll(command, "-", "_")) +
