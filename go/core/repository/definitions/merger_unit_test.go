...
func TestIsTemplateSkeletonPath_L0(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			path:     "/projects/clie-cli/automation/pwsh-templates/assets/templates/root-configurations/skeleton",
			rootDir:  "/projects/clie-cli",
			expected: true,
		},
		{
			name:     "template skeleton subdirectory should be excluded",
			path:     "/projects/clie-cli/automation/pwsh-templates/assets/templates/root-configurations/skeleton/subdir",
			rootDir:  "/projects/clie-cli",
			expected: true,
		},
		{
			name:     "normal directory should not be excluded",
			path:     "/projects/clie-cli/src/internal/definitions",
			rootDir:  "/projects/clie-cli",
			expected: false,
		},
		{
			name:     "templates directory without skeleton should not be excluded",
			path:     "/projects/clie-cli/automation/pwsh-templates/assets/templates/root-configurations",
			rootDir:  "/projects/clie-cli",
			expected: false,
		},
		{
			name:     "skeleton directory not under templates should not be excluded",
			path:     "/projects/clie-cli/some/skeleton/dir",
			rootDir:  "/projects/clie-cli",
			expected: false,
		},
		{
			name:     "Windows path - template skeleton should be excluded",
			path:     "C:\\projects\\clie-cli\\automation\\pwsh-templates\\assets\\templates\\clie-cli\\skeleton",
			rootDir:  "C:\\projects\\clie-cli",
			expected: true,
		},
		{
