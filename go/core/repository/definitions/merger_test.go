...
func TestIsTemplateSkeletonPath(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			path:     "/projects/clie/automation/pwsh-templates/assets/templates/root-configurations/skeleton",
			rootDir:  "/projects/clie",
			expected: true,
		},
		{
			name:     "template skeleton subdirectory should be excluded",
			path:     "/projects/clie/automation/pwsh-templates/assets/templates/root-configurations/skeleton/subdir",
			rootDir:  "/projects/clie",
			expected: true,
		},
		{
			name:     "normal directory should not be excluded",
			path:     "/projects/clie/src/internal/definitions",
			rootDir:  "/projects/clie",
			expected: false,
		},
		{
			name:     "templates directory without skeleton should not be excluded",
			path:     "/projects/clie/automation/pwsh-templates/assets/templates/root-configurations",
			rootDir:  "/projects/clie",
			expected: false,
		},
		{
			name:     "skeleton directory not under templates should not be excluded",
			path:     "/projects/clie/some/skeleton/dir",
			rootDir:  "/projects/clie",
			expected: false,
		},
		{
			name:     "Windows path - template skeleton should be excluded",
			path:     "C:\\projects\\clie\\automation\\pwsh-templates\\assets\\templates\\clie\\skeleton",
			rootDir:  "C:\\projects\\clie",
			expected: true,
		},
	}
