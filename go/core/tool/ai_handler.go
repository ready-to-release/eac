// Package tool provides an AI-specific handler that wraps the AI adapter.
package tool
...
func NewAIToolHandler(toolDef *ToolDefinition) *AIToolHandler {
...
func NewAIToolHandlerWithType(toolDef *ToolDefinition, analysisType AIAnalysisType) *AIToolHandler {
	return &AIToolHandler{
		toolDef:      toolDef,
		analysisType: analysisType,
	}
...
func (h *AIToolHandler) Build(module interfaces.ModuleContractPort, workspaceRoot, outputDir string,
...
func (h *AIToolHandler) loadInput(module interfaces.ModuleContractPort, workspaceRoot, component string) (string, error) {
	moniker := module.GetMoniker()

	switch h.analysisType {
	case AIAnalysisTypeDSL:
		return h.loadDSLContent(workspaceRoot, moniker)
	case AIAnalysisTypeSpecs:
		return h.loadSpecsContent(workspaceRoot, moniker)
...
func (h *AIToolHandler) loadDSLContent(workspaceRoot, moniker string) (string, error) {
	designDir := filepath.Join(workspaceRoot, "specs", moniker, ".design")
	return h.loadFilesWithExtension(designDir, ".dsl")
}

// loadSpecsContent loads Gherkin specification files for a module.
func (h *AIToolHandler) loadSpecsContent(workspaceRoot, moniker string) (string, error) {
	specsDir := filepath.Join(workspaceRoot, "specs", moniker)
	return h.loadFilesWithExtension(specsDir, ".feature")
}

// loadSourceContent loads source code from the module's component roots.
func (h *AIToolHandler) loadSourceContent(module interfaces.ModuleContractPort, workspaceRoot string) (string, error) {
...
func (h *AIToolHandler) ListArtifacts(module interfaces.ModuleContractPort, workspaceRoot string) []string {
...
func (h *AIToolHandler) ValidateModule(module interfaces.ModuleContractPort, workspaceRoot, component string) error {
