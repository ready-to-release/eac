...
func Load(opts LoadOptions) (*EACConfig, error) {
	...
	// Get the global cache (initialized on first call via ConfigManager)
	cache := defaultManager.Cache()

	// Check cache first
	if cachedCfg, ok := cache.Get(repoRoot, opts.ValidateSchemas); ok {
	...
	cache.Set(repoRoot, opts.ValidateSchemas, cfg)
...
// - Merges module-level artifacts (from repository.yml) with type-level artifacts (from component-kinds defaults)
