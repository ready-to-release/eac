...
func TestBuildEnvironmentVars_BaseVars(t *testing.T) {
	...
	assert.Contains(t, envVars, "R2R_DOCKER_MODE=true")
	assert.Contains(t, envVars, "R2R_CONTAINER_REPOROOT=/var/task")
	assert.Contains(t, envVars, "R2R_HOST_REPOROOT=/test/root")

	// Should have HOST_GOOS and HOST_GOARCH
	hasGOOS := false
	hasGOARCH := false
	for _, env := range envVars {
		if len(env) >= 14 && env[:14] == "R2R_HOST_GOOS=" {
			hasGOOS = true
		}
		if len(env) >= 16 && env[:16] == "R2R_HOST_GOARCH=" {
			hasGOARCH = true
		}
	}
	assert.True(t, hasGOOS, "Should include R2R_HOST_GOOS")
	assert.True(t, hasGOARCH, "Should include R2R_HOST_GOARCH")
}
...
func TestBuildEnvironmentVars_TerminalDimensions(t *testing.T) {
	...
	for _, env := range envVars {
		...
		if len(env) >= 22 && env[:22] == "R2R_TERMINAL_DETECTION" {
			hasDetection = true
		}
	}
	assert.True(t, hasCols, "Should include COLUMNS")
	assert.True(t, hasLines, "Should include LINES")
	assert.True(t, hasDetection, "Should include R2R_TERMINAL_DETECTION")
}
