...
func TestNewDockerClient_UsesRealClientByDefault(t *testing.T) {
	...
	assert.True(t, ok, "expected RealDockerClient when CLIE_MOCK_DOCKER is not set")
}

func TestNewDockerClient_UsesMockWhenFlagSet(t *testing.T) {
	// Set mock flag
	os.Setenv("CLIE_MOCK_DOCKER", "true")
	defer os.Unsetenv("CLIE_MOCK_DOCKER")
	...
	assert.True(t, ok, "expected SimpleMockDockerClient when CLIE_MOCK_DOCKER=true")
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.envValue == "" {
				...
				os.Setenv("CLIE_MOCK_DOCKER", tt.envValue)
			}
			defer os.Unsetenv("CLIE_MOCK_DOCKER")
