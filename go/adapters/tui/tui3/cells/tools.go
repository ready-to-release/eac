...
func (c *ToolsCell) SetTools(plannedContainers, activeContainers, plannedSystem, activeSystem []string) {
	c.plannedContainers = plannedContainers
	c.activeContainers = activeContainers
...
func (c *ToolsCell) Render(width, height int) string {
	...
	for _, name := range c.activeContainers {
	...
	for _, name := range c.plannedContainers {
