...
// Each message type is dispatched to a dedicated handler method for readability.
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case batchLineMsg:
		return m.handleBatchLines(msg)
	case lineMsg:
		return m.handleLine(msg)
	case PhaseLineMsg:
		return m.handlePhaseLine(msg)
	case ResultLineMsg:
		return m.handleResultLine(msg)
	case ConfigReadyMsg:
		return m.handleConfigReady(msg)
	case InitSummaryMsg:
		return m.handleInitSummary(msg)
	case SummaryDataMsg:
		return m.handleSummaryData(msg)
	case PlannedWorkMsg:
		return m.handlePlannedWork(msg)
	case UoWEnrichMsg:
		return m.handleUoWEnrich(msg)
	case AllWorkDoneMsg:
		return m.handleAllWorkDone(msg)
	case PhaseUpdateMsg:
		return m.handlePhaseUpdate(msg)
	case statusMsg:
		return m.handleStatus(msg)
	case MarqueeTickMsg:
		return m.handleMarqueeTick(msg)
	case tickMsg:
		return m.handleTick(msg)
	case linesDoneMsg:
		return m.handleLinesDone(msg)
	case statusDoneMsg:
		return m.handleStatusDone(msg)
	case completedMsg:
		return m.handleCompleted(msg)
	case UoWStartMsg:
		return m.handleUoWStart(msg)
	case UoWRunningMsg:
		return m.handleUoWRunning(msg)
	case UoWCompleteMsg:
		return m.handleUoWComplete(msg)
	case TabSelectMsg:
		return m.handleTabSelect(msg)
	case TabDecayMsg:
		return m.handleTabDecay(msg)
	}
...
// handleWindowSize responds to terminal resize events by updating dimensions
// and clamping scroll offsets.
...
// ---------------------------------------------------------------------------
// Line / output messages
// ---------------------------------------------------------------------------

// handleBatchLines processes a batch of output lines from the orchestrator.
func (m Model) handleBatchLines(msg batchLineMsg) (tea.Model, tea.Cmd) {
	lines := []Line(msg)
...
// handleLine processes a single output line (kept for compatibility).
func (m Model) handleLine(msg lineMsg) (tea.Model, tea.Cmd) {
...
// handlePhaseLine writes a line to a specific phase's buffer.
func (m Model) handlePhaseLine(msg PhaseLineMsg) (tea.Model, tea.Cmd) {
...
// handleResultLine writes a line to the results buffer.
func (m Model) handleResultLine(msg ResultLineMsg) (tea.Model, tea.Cmd) {
	m.Execution.ResultsBuffer.Push(msg.Line)
	return m, nil
}

// handleLinesDone marks the line channel as closed.
func (m Model) handleLinesDone(_ linesDoneMsg) (tea.Model, tea.Cmd) {
	m.Execution.LinesDone = true
	return m, nil
}

// ---------------------------------------------------------------------------
// Boot / configuration messages
// ---------------------------------------------------------------------------

// handleConfigReady stores configuration metadata and initializes capacity values.
func (m Model) handleConfigReady(msg ConfigReadyMsg) (tea.Model, tea.Cmd) {
...
// handleInitSummary stores the init summary and reconciles planned tabs with
// the resolved execution tree.
func (m Model) handleInitSummary(msg InitSummaryMsg) (tea.Model, tea.Cmd) {
...
// handlePlannedWork creates grey skeleton tabs from module discovery (before
// tool resolution).
func (m Model) handlePlannedWork(msg PlannedWorkMsg) (tea.Model, tea.Cmd) {
...
// ---------------------------------------------------------------------------
// UoW lifecycle messages
// ---------------------------------------------------------------------------

// handleUoWEnrich enriches an existing planned tab with tool resolution data.
func (m Model) handleUoWEnrich(msg UoWEnrichMsg) (tea.Model, tea.Cmd) {
...
// handleUoWStart creates module state when a module starts (pending state).
func (m Model) handleUoWStart(msg UoWStartMsg) (tea.Model, tea.Cmd) {
	m.GetOrCreateUoWStateByID(msg.Moniker, msg.DisplayName, msg.Weight)
	return m, nil
}

// handleUoWRunning marks a module as running (slot acquired).
func (m Model) handleUoWRunning(msg UoWRunningMsg) (tea.Model, tea.Cmd) {
...
// handleUoWComplete marks a module as complete and optionally auto-selects the
// next running tab.
func (m Model) handleUoWComplete(msg UoWCompleteMsg) (tea.Model, tea.Cmd) {
...
// handleCompleted updates display when a module completes (legacy message).
...
// handlePhaseUpdate updates phase status, timing, and transitions between phases.
func (m Model) handlePhaseUpdate(msg PhaseUpdateMsg) (tea.Model, tea.Cmd) {
...
// ---------------------------------------------------------------------------
// Status and orchestrator messages
// ---------------------------------------------------------------------------

// handleStatus processes orchestrator status updates including running modules,
// capacity tracking, lock info, and Docker metrics.
...
// handleStatusDone marks the status channel as closed.
func (m Model) handleStatusDone(_ statusDoneMsg) (tea.Model, tea.Cmd) {
	m.Execution.StatusDone = true
	return m, nil
}

// ---------------------------------------------------------------------------
// Summary and exit messages
// ---------------------------------------------------------------------------

// handleSummaryData processes the completed summary from the background renderer
// and decides whether to exit immediately or defer to the tick handler.
func (m Model) handleSummaryData(msg SummaryDataMsg) (tea.Model, tea.Cmd) {
...
// handleAllWorkDone signals that all work including AfterExecute hooks is complete.
func (m Model) handleAllWorkDone(_ AllWorkDoneMsg) (tea.Model, tea.Cmd) {
...
// ---------------------------------------------------------------------------
// Tick and animation messages
// ---------------------------------------------------------------------------

// handleMarqueeTick advances the hovered-tab marquee scroll animation.
func (m Model) handleMarqueeTick(_ MarqueeTickMsg) (tea.Model, tea.Cmd) {
...
// handleTick processes the periodic tick: updates metrics, manages exit logic,
// and drives the three-thread model (user timer, summary renderer, exit decision).
...
// ---------------------------------------------------------------------------
// Tab interaction messages
// ---------------------------------------------------------------------------

// handleTabSelect switches to the selected tab and resets scroll position.
func (m Model) handleTabSelect(msg TabSelectMsg) (tea.Model, tea.Cmd) {
...
// handleTabDecay cleans up decayed tabs.
func (m Model) handleTabDecay(_ TabDecayMsg) (tea.Model, tea.Cmd) {
	m.CleanupDecayedTabs()
	return m, nil
}
...
// ---------------------------------------------------------------------------
// Summary and tab finalization helpers
// ---------------------------------------------------------------------------
