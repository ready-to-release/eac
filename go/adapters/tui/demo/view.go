...
// Layout constants
const (
	resourcesHeight = 3 // Three rows for resources (3x3 grid)
...
// renderResourcesPane renders the top resources section as a 3x3 grid.
// Row 1: Work lamps | CPU lamps | Container lamps
// Row 2: Timer | Mem | DockerMem
// Row 3: Counts | Tools (spans two columns)
func (m Model) renderResourcesPane(l Layout) string {
	colWidth := (l.Width - 6) / 3 // -6 for two separators and borders

	// Row 1: Work lamps | CPU lamps | Container lamps
	row1Parts := []string{
		m.cells.UoWStats.RenderLampsOnly(colWidth, 1),
		m.cells.CPU.Render(colWidth, 1),
		m.cells.Containers.Render(colWidth, 1),
	}
	row1 := strings.Join(row1Parts, " │ ")

	// Row 2: Timer | Mem | DockerMem
	row2Parts := []string{
		m.cells.Timer.Render(colWidth, 1),
		m.cells.Mem.Render(colWidth, 1),
		m.cells.DockerMem.Render(colWidth, 1),
	}
	row2 := strings.Join(row2Parts, " │ ")

	// Row 3: Counts | Tools (spans two columns)
	toolsWidth := colWidth*2 + 3 // two columns plus separator
	row3Parts := []string{
		m.cells.UoWStats.RenderCountsOnly(colWidth, 1),
		m.cells.Tools.Render(toolsWidth, 1),
	}
