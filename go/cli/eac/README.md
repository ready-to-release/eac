...
This package is the `main` binary of the `eac-cli` module. It contains no
...
- `main.go`: hand-rolled bubble sort used in `getSubcommands` (line 218) and `printUsage` (line 267) instead of `slices.Sort`
...
- Replace manual sort loops with `slices.Sort` from the standard library -- trivial change
