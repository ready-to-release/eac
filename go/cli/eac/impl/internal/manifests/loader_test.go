...
func TestLoadAllTestManifests_SingleModule(t *testing.T) {
	...
	tmpDir := t.TempDir()
	testDir := filepath.Join(tmpDir, "out", "test", "core")
	...
	if len(manifests) > 0 && manifests[0].Moniker != "core" {
		t.Errorf("Expected moniker 'core', got '%s'", manifests[0].Moniker)
	}
}
...
func TestLoadAllTestManifests_MultipleModules(t *testing.T) {
	...
	modules := []string{"core", "eac-cli", "vscode-commit"}
...
func TestLoadAllTestManifests_SkipsInvalidManifests(t *testing.T) {
	...
	// Valid manifest
	validDir := filepath.Join(tmpDir, "out", "test", "core")
	...
	invalidDir := filepath.Join(tmpDir, "out", "test", "eac-cli")
