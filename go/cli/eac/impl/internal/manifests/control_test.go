...
func TestBuildControlSummary_SingleControl(t *testing.T) {
	...
	if len(entry.Modules) != 1 || entry.Modules[0] != "eac-cli" {
		t.Errorf("Expected modules ['eac-cli'], got %v", entry.Modules)
	}
}
...
func TestBuildControlSummary_AcrossModules(t *testing.T) {
	// Arrange
	manifest1 := implinternal.NewTestManifest("eac-cli", "go", "abc123")
	...
	manifest2 := implinternal.NewTestManifest("vscode-commit", "go", "abc123")
	...
	if !moduleSet["eac-cli"] {
		t.Error("Expected module 'eac-cli' in list")
	}
	if !moduleSet["vscode-commit"] {
		t.Error("Expected module 'vscode-commit' in list")
	}
}
