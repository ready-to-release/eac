// Command: release clie-cli
// Short: Create a git tag for releasing clie-cli using semver format
// Long: Creates a git tag in the format 'clie-cli/x.y.z' to trigger the release workflow.
...
// Long:   - Git tag created in format clie-cli/x.y.z
// Long:   - Tag triggers the release workflow to build and publish binaries
// Long:
// Long: Example: release clie-cli --tag-direct 1.0.0
package release
...
func ReleaseSrcCli() int {
	...
	fs := flag.NewFlagSet("release clie-cli", flag.ExitOnError)
	...
	// When called via go/cli/eac dispatcher, os.Args contains: [binary, "release", "clie-cli", ...args]
	args := os.Args[1:]

	// Remove "release" and "clie-cli" from args to get actual command arguments
	for i, arg := range args {
		if arg == "release" {
			// Found "release", skip it and "clie-cli" (next arg)
			if i+1 < len(args) && args[i+1] == "clie-cli" {
				args = args[i+2:]
				break
			}
		}
	...
	if len(remainingArgs) == 0 {
		...
		log.Errorf("Usage: release clie-cli --tag-direct [--dry-run] [--push=true|false] <version>")
		log.Errorf("Example: release clie-cli --tag-direct 1.0.0")
	...
	if !*tagDirect && !*dryRun {
		...
		log.Errorf("")
		log.Errorf("To tag directly: release clie-cli --tag-direct %s", version)
		return 1
	}

	// Validate module exists
	if err := validateModule("clie-cli"); err != nil {
		log.Errorf("Error: %v", err)
		return 1
	}
	...
	// Build tag name
	tagName := buildTagName("clie-cli", version)
