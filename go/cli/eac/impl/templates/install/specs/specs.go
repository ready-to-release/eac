...
// commandFlags defines valid flags for the templates install specs command
...
func TemplatesInstallSpecs() int {
	...
	log.Debugf("Starting templates install specs command: destination=%s, debug=%v", cfg.Destination, cfg.Debug)
	...
	log.Debugf("Specification templates installed successfully: destination=%s", cfg.Destination)
	log.Infof("✓ Specification templates installed successfully to %s", cfg.Destination)
