...
import (
	...
	"github.com/ready-to-release/eac/go/cli/clie/cmd"
...
func main() {
	...
	if len(originalArgs) != len(filteredArgs) {
		...
		os.Setenv(envconsts.EnvCLIEFixedRedirect, "true")
		os.Setenv(envconsts.EnvCLIEOriginalArgs, argsToString(originalArgs))
		os.Setenv(envconsts.EnvCLIEFilteredArgs, argsToString(filteredArgs))
	}
