...
func TestVersionCommandProperties(t *testing.T) {
	t.Run("command properties", func(t *testing.T) {
		...
		if versionCmd.Short != "Print the version number of clie CLI" {
			t.Errorf("Expected Short description, got %q", versionCmd.Short)
		}

		if !strings.Contains(versionCmd.Long, "clie CLI's") {
			t.Errorf("Expected Long description to contain 'clie CLI's', got %q", versionCmd.Long)
		}
