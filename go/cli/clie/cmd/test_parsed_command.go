...
import (
	...
	commandparser "github.com/ready-to-release/eac/go/cli/clie/internal/command-parser"
)
...
func TestParsedCommandIntegration(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			args:              []string{"clie", "run", "pwsh", "Get-Date"},
			expectedSubcmd:    "run",
			expectedExtension: "pwsh",
		...
		},
		{
			...
			args:              []string{"clie", "--debug", "run", "python", "script.py", "--arg1", "value"},
			expectedSubcmd:    "run",
		...
		},
		{
			...
			args:              []string{"clie", "metadata", "pwsh", "--json"},
...
func TestHelperFunctions(t *testing.T) {
	...
	parsedCommand = &commandparser.ParsedCommand{
		...
		ViperArgs:        []string{"clie", "run", "pwsh"},
		ArgumentBoundary: 3,
	}
	parsedCommandOnce = sync.Once{}
