...
// RootCmd is the base command for the clie CLI when called without any subcommands.
var RootCmd = &cobra.Command{
	Use:   "clie",
	Short: "Ready to Release - Enterprise-grade automation framework",
	Long:  `clie CLI standardizes and containerizes development workflows through a portable, scalable automation framework.`,
	PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
		...
		debug, err := cmd.Flags().GetBool("clie-debug")
		if err != nil {
			return fmt.Errorf("failed to get clie-debug flag: %w", err)
		}
		quiet, err := cmd.Flags().GetBool("clie-quiet")
		if err != nil {
			return fmt.Errorf("failed to get clie-quiet flag: %w", err)
		}
		if debug && quiet {
		...
		// Check CLIE_LOG_LEVEL environment variable first
		if envLogLevel := os.Getenv(envconsts.EnvCLIELogLevel); envLogLevel != "" {
		...
		if os.Getenv(envconsts.EnvCLIEFixedRedirect) == "true" {
			logging.Warnf("Fixed bash redirect pollution in arguments (removed spurious '2' from '2>&1'): original=%s filtered=%s",
				os.Getenv(envconsts.EnvCLIEOriginalArgs), os.Getenv(envconsts.EnvCLIEFilteredArgs))

			// Clean up env vars
			os.Unsetenv(envconsts.EnvCLIEFixedRedirect)
			os.Unsetenv(envconsts.EnvCLIEOriginalArgs)
			os.Unsetenv(envconsts.EnvCLIEFilteredArgs)
		}
...
func init() {
	...
	if os.Getenv(envconsts.EnvCLIEVerboseLog) == "true" || os.Getenv(envconsts.EnvCLIELogLevel) == "debug" {
		logging.EnableDebug()
		versionInfo := version.GetInfo()
		logging.Debugf("CLIE CLI initialized: version=%s commit=%s timestamp=%s",
	...
	// Add flags first
	RootCmd.PersistentFlags().Bool("clie-debug", false, "Enable debug logging")
	RootCmd.PersistentFlags().Bool("clie-quiet", false, "Disable all output except errors")
	...
	// Initialize extension aliases for direct execution (e.g., "clie eac" instead of "clie run eac")
...
func buildErrorContext(err error) string {
	...
	for _, name := range []string{"clie-debug", "clie-quiet"} {
