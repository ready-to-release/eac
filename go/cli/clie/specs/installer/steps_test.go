...
// RegisterSteps registers all clie-installer specific step definitions.
...
// Sets __CLIE_TEST_NO_PATH_UPDATE=1 to skip PATH modification.
// Sets __CLIE_TEST_MOCK=1 to skip GitHub API calls and binary downloads.
func isolatePathEnv() []string {
	env := os.Environ()
	// Add marker that installer scripts can check to skip PATH modification
	env = append(env, "__CLIE_TEST_NO_PATH_UPDATE=1")
	// Add marker to skip downloads and use mock binaries
	env = append(env, "__CLIE_TEST_MOCK=1")
	return env
}
...
func checkLatestReleaseHasBinary() {
	...
	if os.Getenv(environments.EnvCLIETestMock) == "1" {
		binaryAvailable = true
		return
	}
	...
	switch runtime.GOOS {
	case "windows":
		expectedBinary = "clie-windows-amd64-upx.exe" // UPX compressed for faster download
	case "linux":
		switch runtime.GOARCH {
		case "arm64":
			expectedBinary = "clie-linux-arm64" // UPX not available for arm64
		default:
			expectedBinary = "clie-linux-amd64-upx" // UPX compressed for faster download
		}
	case "darwin":
		switch runtime.GOARCH {
		case "arm64":
			expectedBinary = "clie-darwin-arm64" // UPX not available for darwin
		default:
			expectedBinary = "clie-darwin-amd64" // UPX not available for darwin
		}
	default:
