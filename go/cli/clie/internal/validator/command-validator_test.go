...
func TestCommandValidator_ValidateCommand(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:      []string{"clie", "--clie-debug", "version"},
			wantValid: true,
		},
		{
		...
		},
		{
			...
			wantValid:     false,
			wantErrors:    1,
			errorContains: "Cannot use both --clie-debug and --clie-quiet",
		},
		{
			name:          "Run without extension",
			args:          []string{"clie", "run"},
			wantValid:     false,
			wantErrors:    1,
...
func TestCommandValidator_GetViperArguments(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:      []string{"clie", "run", "pwsh"},
			wantViper: []string{"clie", "run", "pwsh"},
		},
		{
			name:      "Run with container args",
			args:      []string{"clie", "run", "python", "script.py", "--verbose"},
...
func TestCommandValidator_GetContainerArguments(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:          []string{"clie", "run", "pwsh", "-Command", "Get-Date | Format-Table"},
...
func TestCommandValidator_IsViperArgument(t *testing.T) {
	...
	args := []string{"clie", "--clie-debug", "run", "python", "script.py", "--verbose"}
