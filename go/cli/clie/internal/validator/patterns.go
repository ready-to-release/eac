...
// Compiled regex patterns from CLIE CLI schema.
var (
	...
	SemanticVersionPattern = regexp.MustCompile(`^v?\d+\.\d+\.\d+(-[a-z0-9.]+)?(\+[a-z0-9.]+)?$|^latest$`)
	VersionPinPattern      = regexp.MustCompile(`^\d+(\.\d+)?(\.\d+)?$|^latest$`)

	// Resource limit patterns.
	MemoryLimitPattern = regexp.MustCompile(`^\d+[kmg]?$`)
	CPULimitPattern    = regexp.MustCompile(`^\d+(\.\d+)?$`)

	// Timeout pattern.
	TimeoutPattern = regexp.MustCompile(`^\d+[smh]$`)
...
// Valid enum values from CLIE CLI schema.
var (
