...
func Validate(forceUpdate bool) error {
	...
	// Check CLIE_NO_UPDATE_CHECK environment variable
	if os.Getenv(envconsts.EnvCLIENoUpdateCheck) == "true" {
		logging.Debug("Update checks disabled by CLIE_NO_UPDATE_CHECK environment variable")
		return nil
	}
