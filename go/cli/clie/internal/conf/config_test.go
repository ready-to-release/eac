...
func TestMain(m *testing.M) {
	// Set CLIE_TESTING to prevent fatal errors in CI during tests
	os.Setenv(environments.EnvCLIETesting, "true")
...
func TestHasLatestTag(t *testing.T) {
	tests := []struct {
		...
		{"implicit latest - github registry", "ghcr.io/ready-to-release/eac-ext", true},
...
func TestValidatePinnedExtensions(t *testing.T) {
	// CLIE_TESTING is already set by TestMain
...
func TestUserSpecificConfigDiscovery(t *testing.T) {
	...
	// Create .clie directory for config files
	clieDir := filepath.Join(tempDir, ".clie")
	err = os.Mkdir(clieDir, 0755)
	require.NoError(t, err, "Failed to create .clie directory")
  ...
  - name: "dev-ext"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create only the files specified for this test (in .clie directory)
			for _, filename := range tt.filesToCreate {
				configPath := filepath.Join(clieDir, filename)
...
func TestGetConfigFileCandidates(t *testing.T) {
	...
	for i, expectedFile := range expectedFiles {
		expectedPath := filepath.Join(clieDir, expectedFile)
