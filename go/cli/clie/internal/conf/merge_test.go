...
func TestMergeExtensionPartialOverride(t *testing.T) {
	...
	assert.Equal(t, "ghcr.io/ready-to-release/eac-ext:v1.0.0", base.Image, "Image should remain unchanged")
...
func TestMergeConfigsExtensions(t *testing.T) {
	...
	eac := extMap["eac"]
	assert.NotNil(t, eac)
	assert.Equal(t, "ghcr.io/ready-to-release/eac-ext:v1.0.0", eac.Image, "Image should remain from base")
...
`
	...
	assert.Equal(t, "ghcr.io/ready-to-release/eac-ext:v1.0.0", Global.Extensions[0].Image)
