...
var (
	reDockerImageRef    = regexp.MustCompile(`^[a-zA-Z0-9][a-zA-Z0-9._/-]*:[a-zA-Z0-9._-]+$|^[a-zA-Z0-9][a-zA-Z0-9._/-]*$`)
	reEnvVarName        = regexp.MustCompile(`^[A-Z][A-Z0-9_]*$`)
	reHostnameFormat    = regexp.MustCompile(`^[a-zA-Z0-9][a-zA-Z0-9.-]*[a-zA-Z0-9]$`)
	reRegistryPrefix    = regexp.MustCompile(`^[a-zA-Z0-9][a-zA-Z0-9.-/]*[a-zA-Z0-9]$`)
	reMemoryLimitFormat = regexp.MustCompile(`^(\d+(\.\d+)?)\s*([bBkKmMgG][bB]?)$`)
)
...
func validateConfig(cfg *Config) error {
	...
	for i, ext := range cfg.Extensions {
		...
		for j, envVar := range ext.Env {
			...
			if envVar.Name == "" {
				...
				if !reEnvVarName.MatchString(envVar.Name) {
	...
	if cfg.Registry != nil {
		if cfg.Registry.Default != "" {
			...
			if !reHostnameFormat.MatchString(cfg.Registry.Default) {
		...
		if cfg.Registry.Authentication != nil {
			...
			if auth.UsernameEnv != "" && !reEnvVarName.MatchString(auth.UsernameEnv) {
			...
			if auth.TokenEnv != "" && !reEnvVarName.MatchString(auth.TokenEnv) {
	...
	if cfg.Environment != nil {
		...
		for i, secretVar := range cfg.Environment.Secrets {
			...
			if secretVar.Name == "" {
				...
				if !reEnvVarName.MatchString(secretVar.Name) {
			...
			if secretVar.Env == "" {
				...
				if !reEnvVarName.MatchString(secretVar.Env) {
	...
	if cfg.Defaults != nil {
		if cfg.Defaults.Registry != "" {
			...
			if !reRegistryPrefix.MatchString(cfg.Defaults.Registry) {
...
func validateMemoryLimit(limit string) error {
	...
	memoryPattern := reMemoryLimitFormat
