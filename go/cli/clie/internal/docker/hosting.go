...
func NewContainerHost() (*ContainerHost, error) {
	...
	// Override Docker host if CLIE_DOCKER_HOST is set
	if dockerHost := os.Getenv(envconsts.EnvCLIEDockerHost); dockerHost != "" {
		clientOpts = append(clientOpts, client.WithHost(dockerHost))
		logging.Debugf("Using custom Docker host from CLIE_DOCKER_HOST: docker_host=%s", dockerHost)
	}
...
func (ch *ContainerHost) ValidateExtensions() error {
	if len(conf.Global.Extensions) == 0 {
		return fmt.Errorf("config file does not contain any extensions. Please run 'clie init' to initialize the configuration")
	}
	return nil
}
