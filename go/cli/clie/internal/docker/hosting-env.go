...
import (
	...
	"github.com/ready-to-release/eac/go/cli/clie/internal/terminal"
...
func (ch *ContainerHost) BuildEnvironmentVars(ext *ExtensionConfig) []string {
	...
	if existingHostRoot := os.Getenv(envconsts.EnvCLIEHostRepoRoot); existingHostRoot != "" {
	...
	envVars := []string{
		envconsts.EnvCLIEDockerMode + "=true",
		envconsts.EnvCLIEHostGOOS + "=" + runtime.GOOS,
		envconsts.EnvCLIEHostGOARCH + "=" + runtime.GOARCH,
		envconsts.EnvCLIEContainerRepoRoot + "=/var/task",
		envconsts.EnvCLIEHostRepoRoot + "=" + hostRepoRoot,
	}
	...
	if width, height, err := terminal.GetSize(); err == nil && width > 0 && height > 0 {
		...
		envVars = append(envVars, "COLUMNS="+cols, "LINES="+lines, envconsts.EnvCLIETerminalDetection+"=auto")
	} else {
		...
		envVars = append(envVars, "COLUMNS="+cols, "LINES="+lines, envconsts.EnvCLIETerminalDetection+"=default")
	}
