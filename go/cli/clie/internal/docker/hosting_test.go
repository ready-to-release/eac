...
func TestBuildEnvironmentVars(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			shouldContain: []string{
				"CLIE_CONTAINER_REPOROOT=/var/task",
				"CLIE_HOST_REPOROOT=/test/root",
				"NO_COLOR=1",
				"TERM=dumb",
				"FORCE_COLOR=0",
				"CI=true",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Set CLIE_HOST_REPOROOT to test context - isolates from host environment
...
func TestCleanupContainers_ExtensionsOnly(t *testing.T) {
	...
	mockClient.On("ContainerList", mock.Anything, mock.MatchedBy(func(opts container.ListOptions) bool {
		...
		{
			ID:      "ext123456789abcd",
			Names:   []string{"/clie-extension-pwsh"},
			State:   "exited",
