...
- `TestTypeRunner` -- interface for test type-specific runners: discovery, metadata extraction, and execution
...
| File | Purpose |
|---|---|
| `registry.go` | `TestTypeRunner` interface, `TestTypeDescriptor`, global registry with `Register`/`Get`/`AllDescriptors` |
| `streaming.go` | `StreamingRunner` for real-time `go test -json` output parsing and result aggregation |

## Dependencies

- `contracts/core` -- action types and tag filter definitions
- `core/config` -- EAC config for test info extraction
- `core/testing` -- provider interfaces that this package implements
...
### Tech Debt
- `registry.go:78` `TestTypeRunner` interface has 6 methods; `FindTestRoot` and `BuildPackagePath` could be split into a separate `TestPathResolver` interface
- `registry.go:153-158` four mutable package-level vars (`runners`, `descriptors`, `fallback`, `mu`); protected by mutex but still global state
...
- Narrow `TestTypeRunner` to fewer methods for simpler runner implementations (medium effort, requires updating all adapters)
