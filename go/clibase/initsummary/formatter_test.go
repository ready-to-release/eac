...
func TestFormatCompactWithIncremental(t *testing.T) {
	...
	// Format: "Incremental: 1 of 3 changed (45ms)"
	if !strings.Contains(output, "1 of 3 changed") {
		t.Errorf("FormatCompact missing incremental info\n\nGot:\n%s", output)
	}
}

func TestFormatCompactWithTestInfo(t *testing.T) {
	s := New("test").
		SetRequest([]string{"core"}, []string{"core"}).
		SetExecutionContext("local").
...
func TestFormatDetailed(t *testing.T) {
	s := New("build").
		...
		SetExecutionPlan([][]string{{"eac-logging"}, {"core", "eac-cli"}}).
	...
	expectedSections := []string{
		...
		"│ 0",               // Layer 0 row
		"│ 1",               // Layer 1 row
		"eac-logging",       // Module in layer 0
		"core",              // Module in layer 1
		"eac-cli",           // Module in layer 1
		"── Flags ──",
		"tidy-first",
