...
type FlagMetadata struct {
	...
	// Declarative flag metadata (optional, for behavior flag pairs)
	Behavior     string          // "cache", "parallel", "tui", etc. (empty for non-declarative)
	IsEnableFlag bool            // True for --with-X variants, false for --no-X
	PairFlagName string          // Name of the paired flag (--with-cache pairs with --no-cache)
	EnvAware     bool            // True if default depends on execution environment
	EnvDefaults  map[string]bool // Context name -> default value (for env-aware flags)
}

// IsBehaviorFlag returns true if this flag is part of a declarative pair.
func (f *FlagMetadata) IsBehaviorFlag() bool {
	return f.Behavior != ""
}
...
func translateCrossCompilePath(compilePath string) string {
	...
	if strings.HasPrefix(compilePath, linuxCIPrefix) {
		// Path format: /home/runner/work/eac/eac/go/cli/eac/...
		// We need to extract: go/cli/eac/...
