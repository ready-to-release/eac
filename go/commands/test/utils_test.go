...
func TestExtractModuleFromPath(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "go/eac module path",
			input: "/project/go/eac/core/config/test",
			want:  "eac-core",
		},
		{
			name:  "go/clie module path",
			input: "/project/go/clie/commands/release",
			want:  "clie-commands",
		},
		{
			name:  "specs path",
			input: "/project/specs/repository/docs-cache",
			want:  "repository",
		},
		{
			name:  "specs impl eac path",
			input: "/project/go/eac/specs/impl/core/steps_test.go",
			want:  "core",
		},
		{
			name:  "specs impl clie path",
			input: "/project/go/clie/specs/impl/clie/steps_test.go",
			want:  "clie",
		},
		{
			name:  "relative go/eac path",
			input: "go/eac/core/config",
			want:  "eac-core",
		},
		{
			name:  "relative specs path",
			input: "specs/eac/login",
			want:  "eac",
		},
		{
			name:  "no match",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := extractModuleFromPath(tt.input)
			if got != tt.want {
				t.Errorf("extractModuleFromPath(%q) = %q, want %q", tt.input, got, tt.want)
			}
		})
	}
}
