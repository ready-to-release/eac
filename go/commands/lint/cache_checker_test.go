...
// newLintUnit creates a workunit.UnitSpec for lint testing with the given module, component, and tool.
func newLintUnit(module, component, tool string) workunit.UnitSpec {
...
func TestLintCacheVerifier_UoWCacheHit(t *testing.T) {
	cachedTime := time.Date(2025, 6, 15, 10, 30, 0, 0, time.UTC)

	unit := newLintUnit("mod-a", "go", "golangci-lint")
	longname := unit.ID.Longname() // "lint:mod-a:go:golangci-lint"
...
func TestLintCacheVerifier_UoWCacheMiss(t *testing.T) {
	cachedTime := time.Date(2025, 6, 15, 10, 30, 0, 0, time.UTC)

	cachedUnit := newLintUnit("mod-a", "go", "golangci-lint")
	uncachedUnit := newLintUnit("mod-a", "go", "staticcheck")
...
func TestLintCacheVerifier_ModuleFallbackHit(t *testing.T) {
	unit := newLintUnit("mod-b", "go", "golangci-lint")
...
func TestLintCacheVerifier_CompletelyUncached(t *testing.T) {
	unit := newLintUnit("mod-c", "go", "golangci-lint")
...
func TestLintCacheVerifier_ContextCancellation(t *testing.T) {
...
func TestLintCacheVerifier_NilMaps(t *testing.T) {
	unit := newLintUnit("mod-a", "go", "golangci-lint")

	verifier := &LintCacheVerifier{} // all maps nil
...
func TestLintCacheVerifier_CacheTimeReturned(t *testing.T) {
	times := []time.Time{
...
func TestLintCacheVerifier_UoWTakesPrecedenceOverModule(t *testing.T) {
...
func TestLintCacheVerifier_UoWFalseDoesNotBlockModuleFallback(t *testing.T) {
	unit := newLintUnit("mod-a", "go", "golangci-lint")
	longname := unit.ID.Longname()
...
func TestLintCacheVerifier_ModuleFalseIsNotCached(t *testing.T) {
	unit := newLintUnit("mod-a", "go", "golangci-lint")
...
func TestLintCacheVerifier_ContextDeadlineExceeded(t *testing.T) {
...
func TestLintCacheVerifier_MultipleUnits(t *testing.T) {
	now := time.Now().UTC().Truncate(time.Second)
	earlier := now.Add(-1 * time.Hour)

	unitA := newLintUnit("mod-a", "go", "golangci-lint")
	unitB := newLintUnit("mod-a", "go", "staticcheck")
	unitC := newLintUnit("mod-b", "py", "ruff")
	unitD := newLintUnit("mod-c", "js", "eslint")
...
func TestLintCacheVerifier_MissingCacheTimeForCachedUoW(t *testing.T) {
	unit := newLintUnit("mod-a", "go", "golangci-lint")
	longname := unit.ID.Longname()
