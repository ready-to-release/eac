// Package base provides shared command-layer utilities for all command packages.
// It contains orchestration logic, display helpers, and test discovery.
// No command implementations or init() registration.
package base
...
func ValidateArtifactsTargetOnly(
	...
	if moduleView, err := reader.GetModule(core.ActionBuild, targetModule); err == nil && moduleView != nil {
		requestedArtifacts = coreoutput.ExtractRequestedArtifactsFromModuleView(moduleView)
	}
...
func ValidateArtifactsWithDependencies(
	...
	for moniker := range allModules {
		...
		if moniker != targetModule {
			if moduleView, ok := moduleViews[moniker]; ok {
				if platforms := coreoutput.ExtractPlatformsFromModuleView(moduleView); len(platforms) > 0 {
...
func validateSingleModule(
	...
	if result.IsDependency && config.IsContainerModule(module) {
		result.HasBuildArtifacts = false
		result.Summary = &config.ArtifactResolutionSummary{}
		return result
	}
	...
	// Resolve artifacts
	artifacts, summary, err := config.ResolveArtifactsForModuleWithConfig(
	...
	if len(requestedArtifacts) > 0 {
		var filteredArtifacts []config.ResolvedArtifact
		filteredSummary := &config.ArtifactResolutionSummary{}
