...
func printDispatchAndWaitUsage() {
	...
	log.Info("")
	log.Info("Flags:")
	log.Info("  --workflow <name>    Workflow file name to dispatch (e.g., ci-clie.yaml)")
	...
	log.Info("")
	log.Info("Examples:")
	log.Info("  pipeline ci dispatch-and-wait --workflow ci-clie.yaml --ref main")
	log.Info("  pipeline ci dispatch-and-wait --run-id 12345678 --timeout 600")
	log.Info("  pipeline ci dispatch-and-wait --workflow ci-clie.yaml --inputs '{\"version\":\"1.0.0\"}'")
}
