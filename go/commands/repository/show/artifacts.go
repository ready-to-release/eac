...
import (
	...
	showinternal "github.com/ready-to-release/eac/go/commands/repository/show/internal"
...
func showArtifactsForModule(moduleName, targetOS, targetArch string, allPlatforms, missingOnly bool) int {
	...
	// Resolve artifacts
	var results []config.ResolvedArtifact
	var summary *config.ArtifactResolutionSummary
	...
	if allPlatforms {
		...
		for _, plat := range platforms {
			platformResults, _, err := config.ResolveArtifactsForModuleWithConfig(
...
func formatArtifactModeBreakdown(module *config.Module, cfg *config.EACConfig) string {
	...
	allArtifacts := base.DetermineRequestedArtifacts(module, true, cfg)
