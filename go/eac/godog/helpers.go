// Package godog provides shared helpers for godog BDD tests.
//
...
// EnsureIsolatedTestRepository sets up an isolated test repository.
// This creates a temporary directory with a minimal git structure for isolated tests.
// The directory is cleaned up after the test.
func EnsureIsolatedTestRepository(ctx *TestContext) error {
	if ctx.IsolatedDir != "" {
		// Already isolated, just ensure git is set up
		return IsGitRepository(ctx)
	}

	// Try to use the fixture pool if available
	if ctx.FixturePool != nil {
		template := ctx.FixturePool.GetTemplate(ctx.OriginalRepoRoot)
		if template != nil {
	...
	// Fall back to simple temp directory isolation
	tempDir, err := os.MkdirTemp("", "isolated-test-*")
	if err != nil {
		return fmt.Errorf("failed to create temp dir: %w", err)
	}
	ctx.IsolatedDir = tempDir
	ctx.CurrentWorkDir = tempDir
