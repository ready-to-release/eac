...
func TestDinDIntegration_PortAllocation(t *testing.T) {
	...
	port1, err := docker.FindAndReservePort()
	require.NoError(t, err)
	assert.True(t, port1 >= docker.PortRangeStart && port1 <= docker.PortRangeEnd)

	port2, err := docker.FindAndReservePort()
	require.NoError(t, err)
	assert.NotEqual(t, port1, port2, "Allocated ports should be different")

	// Clean up
	docker.ReleasePort(port1)
	docker.ReleasePort(port2)
}
...
func TestDinDIntegration_ConcurrentPortAllocation(t *testing.T) {
	...
	for i := 0; i < numPorts; i++ {
		go func() {
			port, err := docker.FindAndReservePort()
			if err != nil {
				errors <- err
				return
			}
...
func TestDinDIntegration_ParallelImageOperations(t *testing.T) {
	...
	operations := []docker.ImageOperation{
		{
	...
	results := docker.ParallelImageEnsure(ctx, dockerCli, operations, &docker.ParallelImageEnsureOptions{
		MaxConcurrency: 3,
	})
