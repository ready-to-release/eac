...
// registerWorkspacesSteps registers step definitions for show-workspaces command features.
func registerWorkspacesSteps(sc *godog.ScenarioContext, ctx *internal.TestContext) {
	// Setup steps
	...
	// Note: "I am in a git repository" step is already registered in internal/steps.go
	// Note: "I am not in a git repository" step is already registered in internal/steps.go
...
func setupDefaultWorktrees(ctx *internal.TestContext) error {
	ctx.MustBeIsolated()
...
func setupCleanWorktree(ctx *internal.TestContext, relPath string) error {
	ctx.MustBeIsolated()
...
func setupDirtyWorktree(ctx *internal.TestContext, relPath string) error {
	ctx.MustBeIsolated()
...
func ensureGitRepository(ctx *internal.TestContext) error {
	ctx.MustBeIsolated()
...
func ensureInitialCommit(ctx *internal.TestContext) error {
...
func createWorktree(ctx *internal.TestContext, path, branch string) error {
...
func verifyTableHeaders(ctx *internal.TestContext, h1, h2, h3 string) error {
...
func verifyBranchStatus(ctx *internal.TestContext, branch, status string) error {
...
func verifyCommitSHAPresent(ctx *internal.TestContext) error {
	output := ctx.CommandOutput
...
func verifyDebugLogs(ctx *internal.TestContext, logPath string) error {
	ctx.MustBeIsolated()
...
func verifyDebugLogsContainWorktreeInfo(ctx *internal.TestContext) error {
	ctx.MustBeIsolated()
...
func verifyMainWorktreeListed(ctx *internal.TestContext) error {
	output := ctx.CommandOutput
