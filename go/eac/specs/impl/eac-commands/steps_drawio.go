...
// These specs require Docker for the drawio-cli container.
package srccommands
...
// registerDrawioSteps registers step definitions for drawio command features.
func registerDrawioSteps(sc *godog.ScenarioContext, ctx *internal.TestContext) {
	dCtx := &drawioContext{}

	// Background steps
	sc.Step(`^the drawio-cli Docker image is available$`, func() error {
	...
	sc.Step(`^the file "([^"]*)" should contain "([^"]*)"$`, func(filename, text string) error {
		return internal.FileContains(ctx, filename, text)
	})
...
// drawioEnsureImage builds the drawio-cli Docker image if needed.
func drawioEnsureImage(ctx *internal.TestContext, dCtx *drawioContext) error {
	...
	cmd = exec.Command("docker", "image", "inspect", "cli-drawio-cli:latest")
	if err := cmd.Run(); err != nil {
		// Image doesn't exist, build it using ORIGINAL repo root (not isolated dir)
		repoRoot := ctx.OriginalRepoRoot
		dockerfilePath := filepath.Join(repoRoot, "containers", "drawio-cli", "Dockerfile")
		contextPath := filepath.Join(repoRoot, "containers", "drawio-cli")

		cmd = exec.Command("docker", "build", "-t", "cli-drawio-cli:latest", "-f", dockerfilePath, contextPath)
		var stderr bytes.Buffer
		cmd.Stderr = &stderr
		if err := cmd.Run(); err != nil {
			return fmt.Errorf("failed to build drawio-cli image: %w: %s", err, stderr.String())
		}
	}
...
func drawioCreateTestFile(ctx *internal.TestContext, filename, pageName string) error {
...
func drawioCreateXMLFile(ctx *internal.TestContext, filename, extraContent string) error {
  ...
  <diagram name="Test" id="test-diagram">
	...
	return internal.CreateFile(ctx, filename, content)
}
...
func drawioCreateEncodedXMLFile(ctx *internal.TestContext, filename, extraContent string) error {
...
func drawioVerifyPNG(ctx *internal.TestContext, filename string) error {
	path := internal.ResolvePath(ctx, filename)
...
func drawioDecodeAndVerify(ctx *internal.TestContext, dCtx *drawioContext, filename, expectedContent string) error {
