...
// registerPipelineStatusSteps registers step definitions for pipeline status command features.
func registerPipelineStatusSteps(sc *godog.ScenarioContext, ctx *internal.TestContext) {
	// Setup steps
...
func githubActionsWorkflowsExist(ctx *internal.TestContext) error {
...
func allWorkflowsPassing(ctx *internal.TestContext) error {
	// Create a test module - by default all workflows pass in mock
	// No need to set R2R_MOCK_FAILING_WORKFLOW
	return createTestModule(ctx, "test-module", nil)
}

func workflowIsFailing(ctx *internal.TestContext) error {
...
func branchExistsWithWorkflows(ctx *internal.TestContext, branchName string) error {
...
func commitHasWorkflowRuns(ctx *internal.TestContext, commitSHA string) error {
...
func noWorkflowsForCommit(ctx *internal.TestContext) error {
...
func iSeeWorkflowNamesAndStatus(ctx *internal.TestContext) error {
...
func iSeeCommitSHA(ctx *internal.TestContext) error {
	output := ctx.CommandOutput
...
func iSeeSuccessIndicators(ctx *internal.TestContext) error {
	output := ctx.CommandOutput
...
func iSeeFailureIndicators(ctx *internal.TestContext) error {
	output := ctx.CommandOutput
...
func iSeeFailingWorkflowName(ctx *internal.TestContext) error {
	output := ctx.CommandOutput
...
func iSeeStatusForBranch(ctx *internal.TestContext, branchName string) error {
	output := ctx.CommandOutput
...
func iSeeStatusForCommit(ctx *internal.TestContext, commitSHA string) error {
	output := ctx.CommandOutput
...
func iSeeSimilarMessage(ctx *internal.TestContext, message string) error {
	output := ctx.CommandOutput
