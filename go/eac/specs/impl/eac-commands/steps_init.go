...
func registerInitSteps(sc *godog.ScenarioContext, ctx *internal.TestContext) {
	...
	sc.Step(`^a \.r2r/eac/agent-config\.yml file is created$`, func() error {
		return initFileExists(ctx, ".r2r/eac/agent-config.yml")
	})
	sc.Step(`^the \.r2r/eac/agent-config\.yml file contains "([^"]*)"$`, func(content string) error {
		return initFileContains(ctx, ".r2r/eac/agent-config.yml", content)
	})
	sc.Step(`^a reconfiguration message is shown$`, func() error {
		return initOutputContainsAny(ctx, "reconfigur", "already initialized", "Reconfiguring")
	})
	...
	sc.Step(`^a \.r2r/eac/agent-config\.yml file exists with ([^"]*)$`, func(provider string) error {
		content := fmt.Sprintf("provider: %s\n", provider)
		return internal.CreateFile(ctx, ".r2r/eac/agent-config.yml", content)
	})
}
