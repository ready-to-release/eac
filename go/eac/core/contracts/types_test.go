...
// Tests explicit changelog paths, default path fallback, and behavior when Versioning is nil.
func TestBaseContract_GetChangelog(t *testing.T) {
	tests := []struct {
		...
		{
			name: "default path fallback when changelog not specified",
			contract: BaseContract{
				Moniker: "test-module",
				Versioning: &ModuleVersioning{
					Scheme: "SemVer",
					// Changelog not specified - should use default
				},
			},
			want: "release/test-module/CHANGELOG.md",
		},
		{
			name: "default path fallback when changelog is empty string",
			contract: BaseContract{
				Moniker: "my-module",
				Versioning: &ModuleVersioning{
					Scheme:    "CalVer",
			...
			want: "release/my-module/CHANGELOG.md",
		},
		{
			name: "default path when versioning is nil",
			contract: BaseContract{
				Moniker:    "no-version-module",
				Versioning: nil,
			},
			want: "release/no-version-module/CHANGELOG.md",
		},
		{
			name: "explicit path with special characters in moniker",
			contract: BaseContract{
		...
		{
			name: "default path with special characters in moniker",
			contract: BaseContract{
				Moniker: "eac-mcp-commands",
				Versioning: &ModuleVersioning{
					Scheme: "SemVer",
...
// format is consistent across multiple invocations.
func TestBaseContract_GetChangelog_ConsistentDefault(t *testing.T) {
	contract := BaseContract{
		Moniker:    "test-module",
		Versioning: &ModuleVersioning{Scheme: "SemVer"},
	}
...
func TestBaseContract_GetChangelog_EdgeCases(t *testing.T) {
	tests := []struct {
		...
		{
			name: "empty moniker without explicit changelog",
			contract: BaseContract{
				Moniker: "",
				Versioning: &ModuleVersioning{
					Scheme: "SemVer",
