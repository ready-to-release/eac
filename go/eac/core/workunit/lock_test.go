...
func TestLockInfo_FieldsExist(t *testing.T) {
	...
	info := LockInfo{
		PID:       12345,
		StartedAt: now,
		UnitID:    "test:eac-core:go:gotest:unit",
	}

	assert.Equal(t, 12345, info.PID)
	assert.Equal(t, now, info.StartedAt)
	assert.Equal(t, "test:eac-core:go:gotest:unit", info.UnitID)
}
...
func TestLockInfo_MarshalJSON(t *testing.T) {
	...
	assert.Contains(t, jsonStr, "build:eac-core:go:go")
}
...
func TestLockInfo_UnmarshalJSON(t *testing.T) {
	jsonData := `{
		...
		"unit_id": "test:eac-core:go:gotest:unit"
	}`
...
func TestErrLocked_ErrorWithHeldBy(t *testing.T) {
	heldBy := &LockInfo{
		PID:       12345,
		StartedAt: time.Date(2024, 1, 15, 10, 30, 0, 0, time.UTC),
		UnitID:    "test:eac-core:go:gotest",
	}

	err := &ErrLocked{
		UnitID:   "test:eac-core:go:gotest",
		HeldBy:   heldBy,
		LockPath: "out/test/eac-core/go/gotest/lock.json",
	}

	errMsg := err.Error()
	assert.Contains(t, errMsg, "test:eac-core:go:gotest")
