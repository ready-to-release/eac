...
func registerTestResultsSteps(sc *godog.ScenarioContext, ctx *eacgodog.TestContext) {
...
%s
  ...
  "version": "1.0"
	...
	sc.Step(`^test has tags \["([^"]*)", "([^"]*)", "([^"]*)"\]$`, func(tag1, tag2, tag3 string) error {
		...
		// Create manifest in isolated test directory
		return eacgodog.CreateFile(ctx, "out/test/eac-commands/test.manifest.json", string(content))
	})
	...
	sc.Step(`^test manifests exist$`, func() error {
		...
		// Create manifest in isolated test directory
		return eacgodog.CreateFile(ctx, "out/test/eac-core/test.manifest.json", string(content))
	})
...
%s
  ...
  "version": "1.0"
		...
		// Create manifest in isolated test directory
		return eacgodog.CreateFile(ctx, "out/test/eac-commands/test.manifest.json", manifestJSON)
	})
