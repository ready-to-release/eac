...
func TestNewDockerClient_UsesRealClientByDefault(t *testing.T) {
	...
	os.Unsetenv("R2R_MOCK_STRUCTURIZR")
	...
	assert.True(t, ok, "expected RealDockerClient when R2R_MOCK_STRUCTURIZR is not set")
}

func TestNewDockerClient_UsesMockWhenFlagSet(t *testing.T) {
	// Set mock flag
	os.Setenv("R2R_MOCK_STRUCTURIZR", "true")
	defer os.Unsetenv("R2R_MOCK_STRUCTURIZR")
	...
	assert.True(t, ok, "expected SimpleMockDockerClient when R2R_MOCK_STRUCTURIZR=true")
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.envValue == "" {
				...
				os.Setenv("R2R_MOCK_STRUCTURIZR", tt.envValue)
			}
			defer os.Unsetenv("R2R_MOCK_STRUCTURIZR")
