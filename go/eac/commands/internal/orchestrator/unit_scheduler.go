	...
	go func() {
		...
		for _, spec := range work {
			...
			go func(s workunit.UnitSpec) {
				...
				if isCached {
					moniker := us.formatDisplayName(s)

					// 1. Update TUI - tab lights up blue
					if us.tuiConsole != nil {
						us.tuiConsole.MarkModuleCompleteWithCacheInfo(
							moniker,
							-1, // Exit code -1 = cached
							cacheTime,
							"", // No log path for early detection
						)
					}
...
func (us *UnitScheduler) RunUnits(work []workunit.UnitSpec, worker UnitWorkerFunc) []UnitResult {
	...
	for _, w := range work {
		...
		us.tuiMarkQueued(displayName, w.Weight)
	}
	...
	for i := 0; i < poolSize; i++ {
		...
		go func() {
			...
			for {
				...
				us.tuiMarkRunning(us.formatDisplayName(*spec))
...
func (us *UnitScheduler) markUnitComplete(spec workunit.UnitSpec, result *UnitResult) {
	...
	us.tuiMarkCompleted(displayName, result.ExitCode)
}
...
func (us *UnitScheduler) tuiMarkPending(displayName string, weight int) {
	if us.tuiConsole == nil {
		return
	}
	// Create the tab with pending status
	us.tuiConsole.StartModule(displayName, weight)
}
...
func (us *UnitScheduler) tuiMarkQueued(displayName string, weight int) {
	us.tuiMarkPending(displayName, weight)
}
...
func (us *UnitScheduler) executeWorker(spec workunit.UnitSpec, worker UnitWorkerFunc) UnitResult {
	...
	if cacheInfo, ok := us.earlyCached.Load(displayName); ok {
	...
	us.addActiveTool(tool, spec.IsContainer)
	...
	if err := os.MkdirAll(componentOutputDir, 0o755); err != nil {
		...
		us.removeActiveTool(tool, spec.IsContainer)
		return result
	}
...
func (us *UnitScheduler) tuiMarkRunning(displayName string) {
	if us.tuiConsole == nil {
		return
	}
	...
	if _, wasEarlyCached := us.earlyCached.Load(displayName); wasEarlyCached {
		return
	}

	// Update status to running
	us.tuiConsole.MarkModuleRunning(displayName)

	us.tuiMu.Lock()
	us.tuiRunning = append(us.tuiRunning, displayName)
	...
	us.tuiConsole.UpdateStatus(tui.Status{
		Phase:             capitalize(us.config.ActionVerb),
		Running:           running,
		Completed:         completed,
		Total:             total,
		Locks:             us.getLockStatuses(),
		ActiveContainers:  us.getActiveContainersList(),
		UsedContainers:    us.getUsedContainersList(),
		ActiveSystemTools: us.getActiveSystemToolsList(),
		UsedSystemTools:   us.getUsedSystemToolsList(),
	})
}

// tuiMarkCompleted removes a component from running, increments completed, and reports exit code.
// Counter updates happen regardless of TUI console presence - workers are the sole source of truth.
func (us *UnitScheduler) tuiMarkCompleted(displayName string, exitCode int) {
	// TUI visual updates first (matches tuiMarkRunning lock ordering pattern)
	// These calls are non-blocking (use program.Send or select with default)
	if us.tuiConsole != nil {
		...
		if exitCode < 0 {
			...
			cacheTime := us.getCacheTime(moduleName)
			us.tuiConsole.MarkModuleCompleteWithCacheInfo(displayName, exitCode, cacheTime, "")
		} else {
			us.tuiConsole.MarkModuleComplete(displayName, exitCode)
		}
	}

	// Update counters under lock (always, even without TUI)
	// Workers are the sole source of truth for completion counting.
	...
	// Final status update (after lock released, matches tuiMarkRunning pattern)
	if us.tuiConsole != nil {
		us.tuiConsole.UpdateStatus(tui.Status{
			Phase:             capitalize(us.config.ActionVerb),
			Running:           running,
...
func (us *UnitScheduler) getLockStatuses() []tui.LockStatus {
	if us.registry == nil {
		return nil
	}
