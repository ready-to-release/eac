...
// File: go/eac/commands/impl/create/design/create_test.go
package design
...
func TestParseConfig(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args:    []string{"cmd", "design", "create", "r2r-cli"},
			wantErr: false,
			checkFunc: func(t *testing.T, config *DesignConfig) {
				if config.Module != "r2r-cli" {
					t.Errorf("Module = %q, want %q", config.Module, "r2r-cli")
				}
				if config.Debug {
		...
		},
		{
			...
			args:    []string{"cmd", "design", "create", "r2r-cli", "--debug"},
			wantErr: false,
			checkFunc: func(t *testing.T, config *DesignConfig) {
				if !config.Debug {
		...
		},
		{
			...
			args:    []string{"cmd", "design", "create", "r2r-cli", "--force"},
			wantErr: false,
			checkFunc: func(t *testing.T, config *DesignConfig) {
				if !config.Force {
		...
		},
		{
			...
			args:    []string{"cmd", "design", "create", "r2r-cli", "--output", "custom/path.dsl"},
			wantErr: false,
			checkFunc: func(t *testing.T, config *DesignConfig) {
		...
		{
			name:    "with all flags",
			args:    []string{"cmd", "design", "create", "r2r-cli", "-d", "-f", "-o", "out.dsl"},
			wantErr: false,
			checkFunc: func(t *testing.T, config *DesignConfig) {
				if !config.Debug {
					t.Error("Debug should be true")
	...
	tests := []struct {
	...
	}{
		{
			...
			setupFunc: func(root string) error {
				sourcePath := filepath.Join(root, "src", "r2r-cli")
...
func TestBuildContractBasedPrompt(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			config: &DesignConfig{
				Module:     "r2r-cli",
				SourcePath: "/path/to/src/r2r-cli",
			},
			setupFunc: func(root string) error {
