...
func TestExtractTestResults_SingleTest(t *testing.T) {
	...
	if result.Module != "eac-core" {
		t.Errorf("Expected module 'eac-core', got '%s'", result.Module)
	}
...
func TestExtractTestResults_GodogTest_ExtractsControlTags(t *testing.T) {
	...
	manifest.AddTest(implinternal.TestEntry{
		...
		Package:    "specs/eac-commands/create-squash-message",
		Type:       "godog",
		Suite:      "integration",
		Status:     implinternal.TestStatusPassed,
		DurationMs: 1234,
		Tags:       []string{"@L2", "@control:ai-2", "@deps:go", "@control:sa-3"},
		FilePath:   "specs/eac-commands/create-squash-message/specification.feature",
	})
	...
	// Check feature path
	if result.FeaturePath != "specs/eac-commands/create-squash-message/specification.feature" {
...
func TestExtractTestResults_MultipleManifests(t *testing.T) {
	// Arrange
	manifest1 := implinternal.NewTestManifest("eac-core", "go", "abc123")
	...
	manifest2 := implinternal.NewTestManifest("eac-commands", "go", "abc123")
	...
	if moduleCount["eac-core"] != 2 {
		t.Errorf("Expected 2 tests from eac-core, got %d", moduleCount["eac-core"])
	}
	if moduleCount["eac-commands"] != 1 {
		t.Errorf("Expected 1 test from eac-commands, got %d", moduleCount["eac-commands"])
	}
}
...
func TestExtractFeatureName(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			filePath:     "specs/eac-core/logging_audit/specification.feature",
			expectedName: "logging_audit",
		},
		{
			name:         "Feature with custom name",
			filePath:     "specs/eac-commands/show-tests/custom.feature",
		...
		},
		{
			...
			filePath:     "C:\\source\\ready-to-release\\eac\\specs\\eac-commands\\get-commands\\specification.feature",
			expectedName: "get-commands",
		},
		{
			name:         "Absolute Linux path",
			filePath:     "/app/specs/eac-commands/work-merge/specification.feature",
