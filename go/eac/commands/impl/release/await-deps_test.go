...
func TestDepCIStatus_Struct(t *testing.T) {
	status := DepCIStatus{
		...
		CIWorkflow:    "ci-eac-core.yaml",
		Status:        "success",
		RunID:         12345,
		RunURL:        "https://github.com/org/repo/actions/runs/12345",
	}

	if status.Moniker != "eac-core" {
		t.Errorf("Moniker = %q, want %q", status.Moniker, "eac-core")
	}
	...
	if status.CIWorkflow != "ci-eac-core.yaml" {
		t.Errorf("CIWorkflow = %q, want %q", status.CIWorkflow, "ci-eac-core.yaml")
	}
}
