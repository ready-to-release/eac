...
func TestLayerOrdering(t *testing.T) {
	...
	layers := [][]PendingModule{
		{
			{Module: "eac-core", Version: "1.0.0", Type: "semver"},
		},
		{
			{Module: "eac-commands", Version: "1.1.0", Type: "semver"},
		},
		{
			{Module: "r2r-cli", Version: "2.0.0", Type: "semver"},
		},
	}
	...
	if layers[0][0].Module != "eac-core" {
		t.Errorf("expected eac-core in layer 0, got %s", layers[0][0].Module)
	}

	// Verify last layer contains the top-level module
	if layers[2][0].Module != "r2r-cli" {
		t.Errorf("expected r2r-cli in layer 2, got %s", layers[2][0].Module)
	}
}
