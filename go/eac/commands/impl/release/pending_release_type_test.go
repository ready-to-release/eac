...
func TestFindModulesWithChangelogsAll(t *testing.T) {
	...
	hasInternal := false
	for _, mod := range modules {
		if mod == "eac-commands" || mod == "eac-mcp-commands" {
			hasInternal = true
			break
		}
	}
...
func TestFilterModulesByReleaseType(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			shouldExclude: []string{"eac-commands", "eac-mcp-commands"},
		},
		{
			name:          "filter internal only",
			filterType:    "internal",
			shouldInclude: []string{"eac-commands", "eac-mcp-commands", "r2r-installer", "vscode-ext-commit"},
		...
		},
		{
			...
			shouldExclude: []string{"r2r-cli", "eac-commands"},
		},
		{
			name:          "no filter (all)",
			filterType:    "",
			shouldInclude: []string{"r2r-cli", "eac-commands", "docs", "r2r-eac-bundle"},
			shouldExclude: []string{},
		},
	}
