...
func TemplatesInstallReports() int {
	...
	log.Debugf("Starting templates install reports command: destination=%s, debug=%v", config.Destination, config.Debug)
	...
	log.Debugf("Report templates installed successfully: destination=%s", config.Destination)
