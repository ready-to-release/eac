...
func TestImportManualCommand_SchemaValidation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			realSchemaPath := filepath.Join(repoRoot, "contracts", "eac-core", "0.1.0", "manual-test-results.schema.json")
...
`
	...
	realExportSchemaPath := filepath.Join(repoRoot, "contracts", "eac-core", "0.1.0", "manual-test-export.schema.json")
