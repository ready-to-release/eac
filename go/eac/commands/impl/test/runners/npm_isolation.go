// npm_isolation.go - NPM isolation helper for TypeScript test execution
//
// This module provides isolated npm environments to prevent:
...
//
// Solution: Copy source files to .cache/npm/work/{module-moniker}/ and run npm ci there.
package runners
...
type NpmIsolation struct {
	...
	workRoot      string // .cache/npm/work (isolated work directories)
	npmCacheDir   string // .cache/npm (npm download cache)
}
...
func NewNpmIsolation(workspaceRoot string) *NpmIsolation {
	return &NpmIsolation{
		...
		workRoot:      filepath.Join(workspaceRoot, ".cache", "npm", "work"),
		npmCacheDir:   filepath.Join(workspaceRoot, ".cache", "npm"),
	}
}
