...
func (r *Registry) Register(pattern string, factory ConsoleFactory) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
// commandPath is space-separated: "build", "work create", "test eac-core".
//
// Resolution order:
...
//  4. Default TUI: "*"
func (r *Registry) NewForCommand(commandPath string, config Config) Console {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *Registry) MustHaveDefault() {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *Registry) ListBindings() []string {
	r.mu.RLock()
	defer r.mu.RUnlock()
