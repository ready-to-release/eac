...
func checkLatestReleaseHasBinary() {
	...
	// Find latest r2r-cli release
	for _, release := range releases {
		if strings.HasPrefix(release.TagName, "r2r-cli/") {
			...
			// Found r2r-cli release but binary not present
	...
	fmt.Printf("[SKIP] No r2r-cli release found on GitHub\n")
	binaryAvailable = false
}
...
func iRunThePowerShellInstaller() error {
	...
	scriptPath := filepath.Join(instCtx.scriptsRoot, "pwsh_scripts", "install.ps1")
...
func iRunTheBashInstaller() error {
	...
	scriptPath := filepath.Join(instCtx.scriptsRoot, "bash_scripts", "install.sh")
