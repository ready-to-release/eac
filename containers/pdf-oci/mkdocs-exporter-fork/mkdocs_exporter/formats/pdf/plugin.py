...
class Plugin(BasePlugin[Config]):
  ...
  def __init__(self):
    ...
    self._allowed_pages: Optional[set] = None  # Pages to render (None = all)
  ...
  def on_page_markdown(self, markdown: str, page: Page, config: Config, **kwargs) -> str:
    ...
    # Skip cover processing for pages beyond limit (optimization)
    if self._allowed_pages is not None and page.file.src_path not in self._allowed_pages:
      return markdown
  ...
  def on_pre_build(self, **kwargs) -> None:
    ...
    self._allowed_pages = None  # Reset for new build
  ...
  def on_pre_page(self, page: Page, config: dict, **kwargs):
    ...
    # Skip PDF setup for pages beyond limit (optimization)
    if self._allowed_pages is not None and page.file.src_path not in self._allowed_pages:
      return
  ...
  def _on_post_build_2(self, config: dict, **kwargs) -> None:
    ...
    # Filter to only pages with PDF format (pages beyond limit don't have it)
    ...
    # Queue all files for parallel merge (don't delete yet - needed during merge)
    aggregate_files = []
    for page in self.pages:
      aggregate_path = page.formats['pdf']['path'] + '.aggregate'
      # Pass page title for TOC
      title = page.title if hasattr(page, 'title') else ''
      self.aggregator.append(aggregate_path, title)
      aggregate_files.append(aggregate_path)

    # Parallel tree-merge happens here
    self.aggregator.save()

    # Cleanup source aggregate files after merge complete
  ...
  def on_nav(self, nav, **kwargs):
    ...
    # Build allowed pages set for page_limit (used by on_pre_page and on_page_markdown)
    page_limit = self.config.get('page_limit', 0)
    if page_limit > 0:
      logger.info("[mkdocs-exporter.pdf] Page limit: %d of %d pages will render PDFs (--artifacts reduced mode)",
                  page_limit, len(self.pages))
      self._allowed_pages = {p.file.src_path for p in self.pages[:page_limit]}
    else:
      self._allowed_pages = None  # All pages allowed
