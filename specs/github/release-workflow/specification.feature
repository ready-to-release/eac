...
Background:
  ...
  And the r2r-cli module is built

Rule: Workflow triggers on release tags matching pattern

  @skip:wip @L3 @iv @deps:git @depm:r2r-cli
  Scenario: Workflow triggers when tag matches r2r-cli pattern
    Given a git repository with r2r-cli source code
    When a git tag matching pattern "r2r-cli/*" is created
...
Rule: Go binary is built for multiple platforms
  ...
  @skip:wip @L3 @iv @deps:go @depm:r2r-cli
...
Rule: GitHub release is created with built artifacts
  ...
  Scenario: GitHub release created with all platform binaries attached
    ...
    Then a release is created with tag "r2r-cli/1.2.3"
