...
# Architecture: Affects eac-cli get squash-message command; reads branch commits ahead of base via git; invokes AI with .eac/ai/commit-message config; outputs conventional commit format; defaults base to main branch

@L2 @deps:go @deps:git @ov @env:isolated-test-project @control:ai-2
Feature: eac-cli_get-squash-message
  ...
  Rule: Requires commits ahead of base branch
    ...
    Scenario: Fail when no commits ahead of base
      ...
      When I run "get squash-message --base=main"
