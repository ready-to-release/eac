...
Feature: repository_docs-structurizr-cache
  ...
  I want to ensure all Structurizr diagrams are cached
  So that CI builds are fast and architecture diagrams render consistently

  Background:
    Given the repository root exists

  Rule: All Structurizr views must have cached SVG files

    @L1 @ov
    Scenario: All workspace.dsl views have cached SVGs
