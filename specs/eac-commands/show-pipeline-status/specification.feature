...
Feature: eac-commands_show-pipeline-status
  ...
  Rule: Command must be registered and accessible
    ...
    Scenario: Command is listed in available commands
      ...
      And I should see "show pipeline-status"

    Scenario: Command has proper description
      When I run the command "show help show pipeline-status"
      Then the exit code is 0
  ...
  Rule: Shows status for main branch HEAD by default
    ...
    Scenario: Show status for main branch
      ...
      When I run "show pipeline-status"
      Then the exit code is 0
  ...
  Rule: Can check specific ref or commit
    ...
    Scenario: Show status for specific branch
      ...
      When I run "show pipeline-status --ref develop"
      Then the exit code is 0
    ...
    Scenario: Show status for specific commit
      ...
      When I run "show pipeline-status --commit abc123"
      Then the exit code is 0
  ...
  Rule: Error handling
    ...
    Scenario: Handle invalid ref
      When I run "show pipeline-status --ref nonexistent-branch"
      Then the exit code is 1
