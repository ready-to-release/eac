...
Feature: security_sast
  ...
  Rule: SAST scanner creates evidence file with integrity verification
    ...
    Scenario: Perform SAST scan with mocked tool
      When I run the command "security sast src-core"
      Then the exit code is 0
      And evidence files should exist in directory "out/security/src-core/sast/"
    ...
    Scenario: SAST scan with specific ruleset
      When I run the command "security sast src-core --config p/security-audit"
      Then the exit code is 0
      And evidence files should exist in directory "out/security/src-core/sast/"

    Scenario: SAST scan with debug logging
      When I run the command "security sast src-core --debug"
      Then the exit code is 0
