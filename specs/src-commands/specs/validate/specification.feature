...
Feature: src-commands_specs-validate
  ...
  Rule: Single file validation uses contract-based validator
    ...
    Scenario: Validate specification with errors
      ...
      And stdout contains "Missing Rule: declaration (required for acceptance criteria)"
  ...
  Rule: Validation uses same logic as create command
    ...
    Scenario: Check Rule requirements
      ...
      And stderr contains "required for acceptance criteria"
    ...
    Scenario: Check Scenario requirements
      ...
      And stderr contains "required for behavior examples"
