...
Feature: src-commands_run-pipeline
  ...
  Rule: Command must be registered and accessible
    ...
    Scenario: Command is listed in available commands
      ...
      And I should see "run pipeline"

    Scenario: Command has proper description
      When I run the command "show help run pipeline"
      Then the exit code is 0
  ...
  Rule: Pipeline executes modules in dependency order
    ...
    Scenario: Run pipeline for all modules
      Given modules exist with dependencies:
        ...
        | src-core   |            |
        | src-cli    | src-core   |
      When I run "run pipeline"
      Then the exit code is 0
      And "src-core" is processed before "src-cli"

    @skip:wip
    Scenario: Run pipeline for specific modules
      Given modules "src-core" and "src-cli" exist
      When I run "run pipeline src-cli"
      Then the exit code is 0
      And only "src-cli" and its dependencies are processed

  Rule: Changed-only mode filters to modified modules

    @skip:wip
    Scenario: Run only changed modules
      Given module "src-core" has uncommitted changes
      And module "src-cli" has no changes
      When I run "run pipeline --changed-only"
      Then the exit code is 0
      And only "src-core" is processed

    @skip:wip
    Scenario: Compare against specific ref
      Given module "src-core" was changed since "main"
      When I run "run pipeline --changed-only --ref main"
      Then the exit code is 0
      And "src-core" is included in the pipeline
  ...
  Rule: Wait flag polls for completion
    ...
    Scenario: Wait with timeout
      Given a pipeline is running
      When I run "run pipeline --wait --timeout 60"
  ...
  Rule: Error handling
    ...
    Scenario: Handle module not found
      When I run "run pipeline nonexistent-module"
