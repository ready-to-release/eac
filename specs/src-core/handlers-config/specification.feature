@L1 @ov @depm:src-core
Feature: src-core_handlers-config
  ...
  Rule: Docker handler must provide dockerfile path configuration
    ...
    Scenario: Dockerfile path placeholders are resolved
      Given the handlers are loaded
      When I resolve dockerfile path "containers/{moniker}/Dockerfile" for moniker "ext-eac" with root "src/ext-eac"
      Then the resolved path should be "containers/ext-eac/Dockerfile"
      When I resolve dockerfile path "{root}/Dockerfile" for moniker "ext-eac" with root "src/ext-eac"
      Then the resolved path should be "src/ext-eac/Dockerfile"
